...
const obrasSocialAdd = Joi.object({
  ...
  cuit: Joi.string().min(2).max(10).required(),
  telefono: Joi.string().min(9).max(10).required(),
  email: Joi.string()
  ...
  direccion: Joi.string().min(5).max(30).required(),
  medicos: Joi.array()
    .items(
      Joi.object({
...
const obrasSocialUpdate = Joi.object({
  ...
  cuit: Joi.string().min(2).max(10).optional(),
  telefono: Joi.string().min(9).max(10).optional(),
  email: Joi.string()
    .email({ tlds: { allow: false } })
    .min(6)
    .max(50)
  ...
  direccion: Joi.string().min(5).max(30).optional(),
  medicos: Joi.array()
    .items(
      Joi.object({
...
export const validateInput = (
  ...
  if (Object.keys(req.body).length) {
    if (req.method === "POST") {
      const { error } = obrasSocialAdd.validate(req.body);
      errorJoi = error;
    } else if (req.method === "PUT") {
      const { error } = obrasSocialUpdate.validate(req.body);
      errorJoi = error;
    }
  }
  if (errorJoi) {
